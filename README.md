...
| `output/….epub` | EPUB3, RTL reading order, embedded Dari font, cover, nav + NCX. |
...
`python-bidi`, `ebooklib`, `pillow`, `fonttools`, `pypdf` (all installed from PyPI).
No system TeX/pandoc is required — the pipeline is self-contained.

## 5. Publication status

`project/logs/publication-readiness.md` → **READY FOR PUBLICATION WITH AUTHOR
CONFIRMATIONS**: 39 QA checks pass, 4 non-blocking warnings (ZWNJ on proper nouns like
میزان/میانگین; emoji→typographic mapping; `epubcheck` not runnable without Java;
non-preferred forms intentionally listed in the glossary). The items awaiting the author's
